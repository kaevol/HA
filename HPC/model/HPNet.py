...
class HPNet(pl.LightningModule):
    ...
    def validation_step(self, data, batch_idx):
        ...
        # Evaluate on agent (agent_type == 0 means 'agent')
    ...
    def test_step(self, data, batch_idx):
        ...
        for i in range(num_agents):
            ...
            key = f"{scenario_name}_{case_id}"
            traj = agent_traj[i].cpu().numpy()
            prob = agent_prob[i].tolist()

            self.test_traj_output[key] = traj
            self.test_prob_output[key] = prob
    ...
    def on_test_end(self):
        ...
        with open(os.path.join(output_path, 'test_traj_output.pkl'), 'wb') as f:
            pickle.dump(self.test_traj_output, f)
        with open(os.path.join(output_path, 'test_prob_output.pkl'), 'wb') as f:
            pickle.dump(self.test_prob_output, f)
