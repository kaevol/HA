...
class MapEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        # Polyline embedding: input_dim=2 (length, side type)
        self.p_emb_layer = TwoLayerMLP(input_dim=2, hidden_dim=hidden_dim, output_dim=hidden_dim)
        # Lane embedding: input_dim=1 (length)
        self.l_emb_layer = TwoLayerMLP(input_dim=1, hidden_dim=hidden_dim, output_dim=hidden_dim)

        # Polyline to lane edge embedding: input_dim=4 (length, theta, heading, side_type)
        self.p2l_emb_layer = TwoLayerMLP(input_dim=4, hidden_dim=hidden_dim, output_dim=hidden_dim)
        # Lane to lane edge embedding: input_dim=8 (length, theta, heading, hop, type_one_hot[4])
    ...
    def forward(self, data: Batch) -> torch.Tensor:
        ...
        p_side = data['polyline']['side'].float()
        p_input = torch.stack([p_length, p_side], dim=-1)
        p_embs = self.p_emb_layer(input=p_input)  # [(C1,...,Cb),D]

        l_length = data['lane']['length']
        l_input = l_length.unsqueeze(-1)
        l_embs = self.l_emb_layer(input=l_input)  # [(M1,...,Mb),D]

        # Edge: polyline to lane
        ...
        p2l_edge_vector = transform_point_to_local_coordinate(
            p2l_position_p[p2l_edge_index[0]], 
            p2l_position_l[p2l_edge_index[1]], 
            p2l_heading_l[p2l_edge_index[1]]
        )
        ...
        p2l_edge_attr_type = p2l_type[p2l_edge_index[0]].float()
        ...
        # Predecessor edges with multi-hop
        l2l_predecessor_edge_index = data['lane', 'lane']['predecessor_edge_index']
        if l2l_predecessor_edge_index.size(1) > 0:
        ...
        # Successor edges with multi-hop
        l2l_successor_edge_index = data['lane', 'lane']['successor_edge_index']
        if l2l_successor_edge_index.size(1) > 0:
        ...
        if len(l2l_edge_index) > 0:
            ...
            l2l_edge_attr_input = torch.cat([l2l_edge_attr_length.unsqueeze(-1), l2l_edge_attr_theta.unsqueeze(-1), l2l_edge_attr_heading.unsqueeze(-1), l2l_edge_attr_hop.unsqueeze(-1), l2l_edge_attr_type.float()], dim=-1)
